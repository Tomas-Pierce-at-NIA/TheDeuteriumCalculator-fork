...
###############################################################################################################################
# re-calculate mass shift for each output table and overwrite them
def get_file_name(time_points, is_differential, num_free_replications, num_complex_replications):
    pattern_list = []
    complexity = CON.CONDITION1
    for i in range(len(time_points)):
        for j in range(num_free_replications):
            replication = j
            pattern = (CON.FULL_HDX_OUTPUT + "_" + str(time_points[i]) + "s_" + str(complexity) + "_" + str(replication + 1) + ".csv")
            pattern_list.append(pattern)
    if is_differential == True:
        complexity = CON.CONDITION2
        for i in range(len(time_points)):
           for j in range(num_complex_replications):
               replication = j
               pattern = (CON.FULL_HDX_OUTPUT + "_" + str(time_points[i]) + "s_" + str(complexity) + "_" + str(replication + 1) + ".csv")
               pattern_list.append(pattern)
    return pattern_list
...
def weighted_average_mass_mlf(mz, Intensity, charge, average_mass):
    ...
    charge = float(charge)
    numerator = sum([mz[i]*Intensity[i] for i in range(len(mz))])
    denominator = sum(Intensity)
    if denominator == 0:
        weighted_mz = 0
        weighted_mass_shift = -1
        fit = "Insufficient Match"
    else:
        weighted_mz = numerator/denominator
        weighted_mass_shift = weighted_mz*charge - charge*CON.MASS_OF_HYDROGEN - average_mass
        fit = fit_gaussian(Intensity)
    return weighted_mz, weighted_mass_shift, fit

def recalculate_shift(Peptide_lib):
    Pep_count = Peptide_lib["Sequence"].count()
    print(Pep_count)
    mz_1 = []
    Intensity_1 = []
    list_average_new = []
    list_shift_new = []
    list_Gussian_fit_new = []
    count = 0
    for i in range(Pep_count-1):
        #print(i)
        count = count + 1
        pep_1 = str(Peptide_lib.loc[i, "Sequence"]) + str(Peptide_lib.loc[i, "Charge"])
        pep_2 = str(Peptide_lib.loc[i + 1, "Sequence"]) + str(Peptide_lib.loc[i + 1, "Charge"])
        if pep_1 == pep_2:
            mz_1.append(Peptide_lib.iloc[i, 8])
            Intensity_1.append(Peptide_lib.iloc[i, 9])
        else:
            charge = Peptide_lib.loc[i, "Charge"]
            mz_1.append(Peptide_lib.iloc[i, 8])
            Intensity_1.append(Peptide_lib.iloc[i, 9])
            average_mass = set_average_mass(Peptide_lib.loc[i, "Sequence"])
            Average, shift, Gussian_fit = weighted_average_mass_mlf(mz_1, Intensity_1, charge, average_mass)
            for j in range(count):
                list_average_new.append(Average)
                list_shift_new.append(shift)
                list_Gussian_fit_new.append(Gussian_fit)
            count = 0
            mz_1 = []
            Intensity_1 = []
    count = 0
    mz_1 = []
    Intensity_1 = []
    for i in range(Pep_count-1, 1, -1):
        #print(i)
        count = count + 1
        #print(count)
        pep_1 = str(Peptide_lib.loc[i, "Sequence"]) + str(Peptide_lib.loc[i, "Charge"])
        ...
        if pep_1 == pep_2:
            ...
            for j in range(count):
                ...
                list_Gussian_fit_new.append(Gussian_fit)
            break
    a = len(list_average_new)
    print(a)
    Peptide_lib["Average"] = list_average_new
    Peptide_lib["Shift"] = list_shift_new
...
def main():
    ...
    start_time = datetime.now()
    # Perform peak matching for non_deuterated run
    ...
    Pep_count = df["Sequence"].count()
    for i in range(Pep_count):
        if df.loc[i, "Deuterium"] > 5:
            df.loc[i, "Mz"] = 0
            df.loc[i, "Intensity"] = 0
            df.loc[i, "PpmError"] = 0
